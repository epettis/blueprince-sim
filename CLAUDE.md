...
- All wiki rooms now have records in `rooms.json`, but several still lack their special *behavior*: Mechanarium (gated arms), Planetarium (Telescope planets), Treasure Trove (black box), Closed Exhibit (security puzzle — Paper Crown pickup is simplified to guaranteed), Throne Room, Vault (deposit boxes), Parlor (boxes). The Lost & Found's steal/gift behavior IS modeled (`special_items.py`).
- **Special items**: PR1 landed the item system (inventory, spawning, digging, per-item effects; `implemented: false` records are inert with `meta.blocked_on`). Deferred to PR2: shop purchases (Commissary/Locksmith/Showroom/Gift Shop/Armory/Kitchen), Trading Post trades, Workshop fabrication, item-use actions (Repellent, Royal Scepter), microchips, carry-over wrapper. Deferred to PR3: env observation/action wiring (a retrain point).
