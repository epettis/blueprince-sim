...
class ShopsState:
    ...
    # Give-only (receive: false) items attach as extra sources into that cycle and
            ...
            match tier:
                case 5:
                    if game.rng.chance("trade_graph", t5_special_chance):
                        # Tier-5 special: the Allowance Token or the single
                        # tradeable Upgrade Disk.  15 fixed locations plus that
                        # one disk are the game's whole supply of 16, and it is
                        # unique - once held, the availability walk rejects it
                        # and the trade falls through to the graph's other
                        # outcomes (ultimately dice).
                        successor = ("upgrade_disk_trade"
                                     if game.rng.chance("trade_graph", 0.5)
                                     else "allowance_token")
...
# Sentinels terminate trade-graph walks without resolving to an item id.
# upgrade_disk_trade is a real item id (not a sentinel): it resolves through
# the normal availability walk, so a held disk simply stops being offered.
# Sentinels are the non-item terminals only.
# The 15 fixed-source disks are one-time per source and are never generated by
# the repeatable trade path.
_SENTINELS = frozenset({"dice", "allowance_token"})
...
def _trade_target_ok(state, registry, item_id: str) -> bool:
    ...
    ids, a give-only (receive: false) item, and a second Stopwatch after one
    already ran today.
...
def carryover(game) -> dict:
    ...
    return {
        ...
        # lab_visited: earned the first time the player enters the Laboratory.
        # ORed from state so the config is never mutated, same shape as
        ...
        # conservatory_floorplan_found: earned the first time the player
        # arrives at the campsite holding a shovel. ORed from state so the
        # config is never mutated, same shape as treasure_trove_blackprint.
        # Permanently adds the Conservatory to the draft pool
        # (decks.py::eligible_pool, pool == "found_floorplan") from the
        # following day onward.
        "conservatory_floorplan_found": (
            cfg.conservatory_floorplan_found or state.conservatory_floorplan_found
        ),
        ...
        # Ignition targets permanently lit: accumulated union across all days.
        # Once a target is lit it cannot be lit again on any future day.
        "lit_targets": sorted(
