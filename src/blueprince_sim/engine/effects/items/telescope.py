...
day, and cannot spawn otherwise." Reads ``state.stars_at_day_start`` (the
frozen snapshot seeded from cfg.stars at reset(), before anything today could
grow it), never the live ``state.stars`` -- a star earned mid-day (Observatory/
Starfish Aquarium) must not un-gate the Telescope until TOMORROW's start-of-day
...
def gate(state, gated: list[str]) -> None:
    """Appends ``telescope`` to ``gated`` unless ``state.stars_at_day_start >= 1``."""
    if getattr(state, "stars_at_day_start", 0) < 1:
