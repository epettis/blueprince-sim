...
def all_unlocks_config(reward: str = "shaped") -> GameConfig:
    ...
    return GameConfig(
        ...
        # Both conjuncts of Private Drive -> Blackbridge Grotto: the Laboratory must
        # have been entered AND powered, and each is judged on its own, so setting
        # one alone leaves the edge shut. Save-scoped, so neither is a
        # _CARRYOVER_KEYS flag; set here for the same reason mine_unlocked is --
        # "every permanent unlock", not only the carry flags.
        lab_visited=True,
        lab_powered=True,
